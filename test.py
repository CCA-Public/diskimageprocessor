...
class TestDiskImageProcessorIntegration(SelfCleaningTestCase):
    ...
    def test_integration_processing_tsk(self):
        ...
        subprocess.call(
            "python {} {} {}".format(script, img_source, out_dir), shell=True
        )

        # outputs
        self.assertTrue(is_non_zero_file(j(out_dir, "description.csv")))
        self.assertTrue(is_non_zero_file(j(out_dir, "diskimageprocessor.log")))

        # SIP dir
        self.assertTrue(os.path.isdir(j(out_dir, "SIPs")))
        self.assertTrue(os.path.isdir(j(out_dir, "SIPs", "practical.floppy.dd")))

        # objects
        self.assertTrue(
            os.path.isdir(j(out_dir, "SIPs", "practical.floppy.dd", "objects"))
        )
        self.assertTrue(
            os.path.isdir(
                j(out_dir, "SIPs", "practical.floppy.dd", "objects", "diskimage")
            )
        )
        self.assertTrue(
            is_non_zero_file(
        ...
        self.assertTrue(
            os.path.isdir(j(out_dir, "SIPs", "practical.floppy.dd", "objects", "files"))
        )
        self.assertTrue(
            is_non_zero_file(
                j(out_dir, "SIPs", "practical.floppy.dd", "objects", "files", "ARP.EXE")
            )
        )
        ...
        # metadata
        self.assertTrue(
            os.path.isdir(j(out_dir, "SIPs", "practical.floppy.dd", "metadata"))
        )
        self.assertTrue(
            is_non_zero_file(
                j(out_dir, "SIPs", "practical.floppy.dd", "metadata", "checksum.md5")
            )
        )
        self.assertTrue(
            os.path.isdir(
    ...
    def test_integration_processing_bag(self):
        ...
        out_dir = j(self.dest_tmpdir, "test")
        subprocess.call(
            "python {} -b {} {}".format(script, img_source, out_dir), shell=True
        )
    ...
    def test_integration_processing_bulk_extractor(self):
        ...
        out_dir = j(self.dest_tmpdir, "test")
        subprocess.call(
            "python {} -p {} {}".format(script, img_source, out_dir), shell=True
        )
    ...
    def test_integration_processing_filesonly(self):
        ...
        out_dir = j(self.dest_tmpdir, "test")
        subprocess.call(
            "python {} -f {} {}".format(script, img_source, out_dir), shell=True
        )
        ...
        self.assertTrue(
            is_non_zero_file(
                j(out_dir, "SIPs", "practical.floppy.dd", "objects", "ARP.EXE")
            )
        )
    ...
    def test_integration_processing_exportall(self):
        ...
        out_dir = j(self.dest_tmpdir, "test")
        subprocess.call(
            "python {} -e {} {}".format(script, img_source, out_dir), shell=True
        )
    ...
    def test_integration_analysis_tsk(self):
        ...
        # analysis csv
        self.assertTrue(is_non_zero_file(j(out_dir, "analysis.csv")))
        ...
        # reports
        self.assertTrue(
            is_non_zero_file(j(out_dir, "reports", "practical.floppy.dd", "dfxml.xml"))
        )
        self.assertTrue(
            is_non_zero_file(
                j(out_dir, "reports", "practical.floppy.dd", "disktype.txt")
            )
        )
        self.assertTrue(
            os.path.isdir(j(out_dir, "reports", "practical.floppy.dd", "brunnhilde"))
        )
        self.assertTrue(
            is_non_zero_file(
                j(
    ...
    def test_integration_analysis_keepfiles(self):
        ...
        out_dir = j(self.dest_tmpdir, "test")
        subprocess.call(
            "python {} -k {} {}".format(script, img_source, out_dir), shell=True
        )

        # outputs
        self.assertTrue(os.path.isdir(j(out_dir, "files")))
        self.assertTrue(
            is_non_zero_file(j(out_dir, "files", "practical.floppy.dd", "ARP.EXE"))
        )
    ...
    def test_integration_analysis_exportall(self):
        ...
        out_dir = j(self.dest_tmpdir, "test")
        subprocess.call(
            "python {} -ek {} {}".format(script, img_source, out_dir), shell=True
        )
